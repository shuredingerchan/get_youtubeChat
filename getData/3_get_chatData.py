...
def main():
    ...
    for _ , row_line in streamer_videoId_title_List.iterrows():
        ...
        #ファイル存在してたらとばす
        writeDir = os.getcwd() + "\\chat_list_output\\" + streamer
            ...
            try:
                ...
                for samp in dics["continuationContents"]["liveChatContinuation"]["actions"][1:]:
                    ...
                    if 'item' not in samp["replayChatItemAction"]["actions"][0]["addChatItemAction"]:
                        continue
                    #対象外（スティッカー、メンバーシップ加入、コメント撤回（編集？）
    ...
    membershipPath = writeDir + "\\membership.csv"
    with open(membershipPath, mode='w', encoding="utf-8") as m:
            m.writelines(memberText)           
    print("実行終了")
